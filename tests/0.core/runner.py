#!/usr/bin/env python3
"""
#exonware/xwformats/tests/0.core/runner.py
...
def main():
    """Run core tests. Layer runners stream to stdout/stderr only (no files)."""
    ...
    if USE_XWSYSTEM_UTILS:
        print_header("Core Tests - Fast, High-Value Checks")
        print(f"Directory: {format_path(test_dir)}")
        result = run_pytest(
            test_dir=test_dir,
            markers=["xwformats_core"],
            cwd=root_dir
        )
