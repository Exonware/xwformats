#!/usr/bin/env python3
"""
#exonware/xwformats/tests/3.advance/runner.py
...
def main():
    """Run advance tests. Layer runners stream to stdout/stderr only (no files)."""
    ...
    if USE_XWSYSTEM_UTILS:
        print_header("Advance Tests - Production Excellence Validation")
        print(f"Directory: {format_path(test_dir)}")
        result = run_pytest(
            test_dir=test_dir,
            markers=["xwformats_advance"],
            cwd=root_dir
        )
        ...
        print("=" * 80)
        print("Advance Tests - Production Excellence Validation")
        print("=" * 80)
