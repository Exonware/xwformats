...
# Optional dependency - handle all exceptions to prevent xwlazy retry loops
# Protobuf can fail with AttributeError (version incompatibilities), not just ImportError
try:
    from google.protobuf import message, json_format
    PROTOBUF_AVAILABLE = True
except (ImportError, AttributeError, ModuleNotFoundError, Exception):
    # If protobuf is not available or has version issues, we can't use it
    PROTOBUF_AVAILABLE = False
    message = None
    json_format = None
    # Note: This serializer will fail at runtime if protobuf is not available
    # This is intentional - protobuf requires the library to function
...
class ProtobufSerializer(ASerialization):
    ...
    def encode(self, value: Any, *, options: EncodeOptions | None = None) -> bytes | str:
        ...
        Returns:
            Protobuf bytes
        Raises:
            SerializationError: If encoding fails or protobuf is not available
        """
    ...
    def decode(self, repr: bytes | str, *, options: DecodeOptions | None = None) -> Any:
        ...
        Raises:
            SerializationError: If decoding fails or protobuf is not available
        """
        if not PROTOBUF_AVAILABLE or message is None:
            raise SerializationError(
                "Protobuf library not available. Install with: pip install protobuf",
